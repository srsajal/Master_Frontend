...
import { MasterSCHEMEHEADService } from 'src/app/demo/service/MasterService/master-schemehead.service';
...
export class MasterSchemeHeadFormsComponent implements OnInit {
  ...
  getTcode(){
    this.getTCode = this.userForm.value.minorHeadId;
  }
  ...
  initializeMasterForm(isDisabled: boolean = false): FormGroup {
    const _newForm = this.fb.group({
      minorHeadId: [{ value: this.formMaster?.minorHeadId ?? '', disabled: isDisabled }, Validators.required],
  ...
  submit() {
    if (this.userForm.valid) {
      this.masterService.postmasterSCHEME_HEAD(this.userForm).subscribe((res: masterSchemeHead) => {
        console.log(res);
        this.pgetData();
        this.ref.close();
  ...
  update() {
    if (this.userForm.valid) {
      this.masterService.updatemasterSCHEME_HEAD(this.id, this.userForm).subscribe((res: masterSchemeHead) => {
        console.log(res);
        this.pgetData();
        this.ref.close();
      },

...
export class MasterTreasuryFormsComponent implements OnInit {
  ...
  submit() {
    if (this.userForm.valid) {
      ...
      error => {
        console.error('Error adding MasterTreasuydata:', error);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to add MasterTreasuy data', life: 2000 });
        this.ref.close();
      }
    );
      // form.reset();
  ...
  getDataById() {
    this.masterService.getMasterTreasuryById(this.id).subscribe((res: MasterTreasury) => {
      ...
      error => {
        console.error('Error fetching MasterTreasuydata by ID:', error);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to fetch MasterTreasuydata by ID', life: 2000 });
      }
    );
  }
